...
async def run_agent_with_citations(agent: Agent, user_input: str) -> list[Annotation]:
        ...
        with get_tracer().start_as_current_span("Scenario: Agent Chat Foundry v2", kind=SpanKind.CLIENT) as current_span:
            ...
            try:
                ...
                async for chunk in agent.run(user_input, stream=True):
                    ...
                    # Collect citations from response chunks
                    if hasattr(chunk, 'contents'):
                        for content in chunk.contents:
                            if hasattr(content, 'annotations') and content.annotations:
                                # Convert annotations to simple dicts to avoid serialization issues
    ...
    if citations:
        ...
        for i, citation in enumerate(citations, 1):
            print(f"[{i}] {citation.get('title', 'No title')}: {citation.get('url', '')}")
